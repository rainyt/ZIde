...
var XmlEditorContent = $hx_exports["XmlEditorContent"] = function() {
	...
	this.classesEnd = [];
	this.classes = [];
	var xml = Xml.parse("<tips>\n    <base>\n        <width tips=\"设置宽度\"/>\n        <height tips=\"设置高度\"/>\n        <scaleX tips=\"设置X缩放\"/>\n        <scaleY tips=\"设置Y缩放\"/>\n    </base>\n    <ZImage include=\"base\" tips=\"图像对象\">\n        <src tips=\"设置图像，支持网络地址、图集地址\"/>\n        <width tips=\"设置宽度\"/>\n        <height tips=\"设置高度\"/>\n        <scaleX tips=\"设置X缩放\"/>\n        <scaleY tips=\"设置Y缩放\"/>\n    </ZImage>\n    <ZBox include=\"base\" tips=\"基本容器\">\n        \n    </ZBox>\n    <ZVBox include=\"base\" tips=\"基本容器(竖向)\">\n        \n    </ZVBox>\n    <ZHBox include=\"base\" tips=\"基本容器(横向)\">\n        \n    </ZHBox>\n</tips>");
	var item = xml.firstElement().elements();
	while(item.hasNext()) {
		var item1 = item.next();
		if(item1.exists("tips")) {
			var tmp = this.classes;
			...
			tmp.push(Suggestions.create(item1.nodeName,item1.nodeName,item1.get("tips")));
			var tmp2 = this.classesEnd;
			if(item1.nodeType != Xml.Element) {
				throw haxe_Exception.thrown("Bad node type, expected Element but found " + (item1.nodeType == null ? "null" : XmlType.toString(item1.nodeType)));
			}
			if(item1.nodeType != Xml.Element) {
			...
			tmp2.push(Suggestions.create("/" + item1.nodeName,item1.nodeName,item1.get("tips")));
			if(item1.nodeType != Xml.Element) {
				throw haxe_Exception.thrown("Bad node type, expected Element but found " + (item1.nodeType == null ? "null" : XmlType.toString(item1.nodeType)));
			}
			this.writeAttr(item1.nodeName,item1);
		}
	}
};
XmlEditorContent.__name__ = "XmlEditorContent";
XmlEditorContent.prototype = {
	writeAttr: function(className,xml) {
		var data = [];
		var item = xml.elements();
		while(item.hasNext()) {
			var item1 = item.next();
			if(item1.nodeType != Xml.Element) {
				throw haxe_Exception.thrown("Bad node type, expected Element but found " + (item1.nodeType == null ? "null" : XmlType.toString(item1.nodeType)));
			}
			var tmp = item1.nodeName;
			if(item1.nodeType != Xml.Element) {
				throw haxe_Exception.thrown("Bad node type, expected Element but found " + (item1.nodeType == null ? "null" : XmlType.toString(item1.nodeType)));
			}
			data.push(Suggestions.create(tmp,item1.nodeName + "=\"\"",item1.get("tips")));
		}
		haxe_Log.trace("写入",{ fileName : "XmlEditor.hx", lineNumber : 48, className : "XmlEditorContent", methodName : "writeAttr", customParams : [className,data]});
		Reflect.setProperty(XmlEditorContent.classesAttr,className,data);
	}
	,provideCompletionItems: function(model,position) {
		...
		var className = Std.string(HxOverrides.substr(content,content.indexOf("<") + 1,content.indexOf(" ") - 1));
		var find = Reflect.getProperty(XmlEditorContent.classesAttr,className);
		if(find != null) {
			this.triggerCharacters = [" "];
			return this.returnSuggestions(find);
		}
		if(sym == "/") {
			return this.returnSuggestions(this.classesEnd);
		}
		return this.returnSuggestions(this.classes);
	}
...
Suggestions.create = function(label,insertText,detail,className) {
	if(className == null) {
		className = "Function";
	}
	return { label : label, insertText : insertText, detail : detail, kind : monaco.languages.CompletionItemKind[className]};
};
