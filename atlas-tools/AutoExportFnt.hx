	...
	public function new(file:String, fnt:String, bitmapData:BitmapData) {
		...
		while (ix < bitmapData.width) {
			...
			if (!isAlpha(bitmapData.getPixels(new Rectangle(ix, 0, 1, bitmapData.height))) && ix != bitmapData.width - 1) {
				...
				// 往右边检测10px，看看是否已经是透明，如果存在不透明的情况，则当做同一体来处理。
				var testIsAlpha = true;
				for (i in 0...10) {
					if (!isAlpha(bitmapData.getPixels(new Rectangle(ix + i, 0, 1, bitmapData.height))) && ix + i != bitmapData.width - 1) {
						testIsAlpha = false;
						break;
					}
				}
